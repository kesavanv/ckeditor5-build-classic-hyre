...
/**
 * @module list/mcqoptionsconverters
 */
...
export function modelViewInsertion( model, onCheckboxChecked ) {
	return ( evt, data, conversionApi ) => {
		...
		consumable.consume( modelItem, 'attribute:mcqOptionsChecked' );

		const viewWriter = conversionApi.writer;
		const viewItem = generateLiInUl( modelItem, conversionApi );

		const isChecked = !!modelItem.getAttribute( 'mcqOptionsChecked' );
...
export function dataModelViewInsertion( model ) {
	return ( evt, data, conversionApi ) => {
		...
		if ( data.item.getAttribute( 'mcqOptionsChecked' ) ) {
 ...
 * When a view checkbox element is marked as checked, an additional `mcqOptionsChecked="true"` attribute is added to the model item.
 *
 * It is used by {@link module:engine/controller/datacontroller~DataController}.
 *
...
export function dataViewModelCheckmarkInsertion( evt, data, conversionApi ) {
	...
	if ( data.viewItem.hasAttribute( 'checked' ) ) {
		writer.setAttribute( 'mcqOptionsChecked', true, modelItem );
	}
...
export function modelViewChangeType( onCheckedChange, view ) {
	return ( evt, data, conversionApi ) => {
		...
		if ( data.attributeNewValue == 'todo' ) {
			const isChecked = !!data.item.getAttribute( 'mcqOptionsChecked' );
...
/**
 * A model-to-view converter for the `mcqOptionsChecked` attribute change on the `listItem` model element.
 *
...
export function modelViewChangeChecked( onCheckedChange ) {
	return ( evt, data, conversionApi ) => {
		// Do not convert `mcqOptionsChecked` attribute when to-do list item has changed to other list item.
		// This attribute will be removed by the model post fixer.
		if ( data.item.getAttribute( 'listType' ) != 'todo' ) {
			return;
		}

		if ( !conversionApi.consumable.consume( data.item, 'attribute:mcqOptionsChecked' ) ) {
			return;
		}
