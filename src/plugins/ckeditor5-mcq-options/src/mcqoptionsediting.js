...
import McqOptionsCheckCommand from './mcqoptionscheckcommand';
 ...
 */
export default class McqOptionsEditing extends Plugin {
	/**
	 * @inheritDoc
	 */
	static get pluginName() {
		return 'McqOptionsEditing';
	}
	...
	init() {
		...
		model.schema.extend( 'listItem', {
			allowAttributes: [ 'mcqOptionsChecked' ]
		} );

		// Disallow mcqOptionsChecked attribute on other nodes than listItem with to-do listType.
		model.schema.addAttributeCheck( ( context, attributeName ) => {
			const item = context.last;

			if ( attributeName == 'mcqOptionsChecked' && item.name == 'listItem' && item.getAttribute( 'listType' ) != 'todo' ) {
				return false;
			}
		} );

		// Register commands.
		editor.commands.add( 'mcqOptions', new ListCommand( editor, 'todo' ) );
		editor.commands.add( 'mcqOptionsCheck', new McqOptionsCheckCommand( editor ) );
		...
		editing.downcastDispatcher.on(
			'attribute:mcqOptionsChecked:listItem',
		...
		editor.keystrokes.set( 'Ctrl+space', () => editor.execute( 'mcqOptionsCheck' ) );

		// Remove `mcqOptionsChecked` attribute when a host element is no longer a to-do list item.
		...
		model.document.registerPostFixer( writer => {
			...
			for ( const listItem of listItemsToFix ) {
				writer.removeAttribute( 'mcqOptionsChecked', listItem );
				hasChanged = true;
			}
	 ...
	 * to toggle the `mcqOptionsChecked` attribute using the command, and restores the selection position.
	 *
	...
	_handleCheckmarkChange( listItem ) {
		...
		model.change( writer => {
			...
			editor.execute( 'mcqOptionsCheck' );
